...
# global array that will 'cache' db updates if the other replicas are offline
book_count_array = []
book_cost_array = []

# ===============CREATE CATALOG DATABASE & INSERT LIST OF BOOKS =====================
DEFAULT_PATH = os.path.join(os.path.dirname(__file__), 'catalogdb2.db')
...
def insert_list_of_books():
    available_books = [(666, 'distributed_systems', 'How to get a good grade in 677 in 20 minutes a day', 10, 10),
                       ...
                       (684, 'graduate_school', 'Why Theory Classes are so Hard', 16, 6),
                       (499, 'distributed_systems', 'How to finish Project 3 on time', 7, 10),
                       (846, 'graduate_school', 'Spring in the Pioneer Valley', 4, 15)]

    all_books = query_all_existing_books()

    # if the database doesn't contain all the 7 books, then insert them into the database
    if len(all_books) < 7:
        db_connection = sqlite3.connect(DEFAULT_PATH)
        db_cursor = db_connection.cursor()
...
def update_book_count(book_number, new_stock_count):
    ...
    db_connection.commit()
    db_cursor.close()
    data2send = (book_number, new_stock_count)
    send_to_replica_data("update_book_count", data2send)


# same as above, except that current database is a replica
# book_number, new_stock_count
def update_book_count_replica(data2process):
    db_connection = sqlite3.connect(DEFAULT_PATH)
    for index, row in enumerate(data2process):
        book_number = row[0]
...
def update_cost_of_one_book(book_number, new_book_cost):
    ...
    db_connection.commit()
    db_cursor.close()
    data2send = (book_number, new_book_cost)
    send_to_replica_data("update_cost_one_book", data2send)


# Same as above, except current database is a replica
# book_number, new_book_cost
def update_cost_of_one_book_replica(data2process):
    db_connection = sqlite3.connect(DEFAULT_PATH)
    for index, row in enumerate(data2process):
        book_number = row[0]
        new_book_cost = row[1]
...
app = Flask("catalog2_server")


@app.route('/catalog/check_heartbeat', methods=['GET'])
def heartbeat():
    return jsonify("Yes")


# ========================== FUNCTIONS RELATED TO REPLICAS ======================
# master function that will send data to replicas and perform changes to the database accordingly..
def send_to_replica_data(operation_type, data2send):
    if operation_type == "update_book_count":
        book_count_array.append(data2send)
        try:
            response = requests.get(
                'http://{}:{}/update_replicas/{}/{}'.format(catalog1_ip, catalog1_port, operation_type,
                                                            book_count_array))
            book_count_array.clear()
        except:
            None

    elif operation_type == "update_cost_one_book":
        book_cost_array.append(data2send)
        try:
            response = requests.get(
                'http://{}:{}/update_replicas/{}/{}'.format(catalog1_ip, catalog1_port, operation_type,
                                                            book_cost_array))
            book_cost_array.clear()
        except:
            None
    else:
...
# master function to receive data from replicas as perform changes to the database accordingly
@app.route('/update_replicas/<operation_type>/<data2process>', methods=['GET'])
def receive_from_replica_data(operation_type, data2process):
    data2process = ast.literal_eval(data2process)
    if operation_type == "update_book_count":
        update_book_count_replica(data2process)
    elif operation_type == "update_cost_one_book":
        update_cost_of_one_book_replica(data2process)
    else:
        print("No operation specified !")

    return jsonify("confirmed: received replica data")


# ===================== RESYNC ALL THE DATABASES BEFORE GOING ON LINE ==================
# Resyncs the current database with that of the replica.
#This happens before the server comes online.
# Note that this implements a weak form of database consistency.
def resync_databases():
    try:
        response = requests.get('http://{}:{}/resync_replicasdbs/{}'.format(catalog1_ip, catalog1_port, "all"))
        print("Resyncing databases...")
        bk_count = response.json()['book_count_array']
        bk_cost = response.json()['book_cost_array']
        if len(bk_count) != 0:
            update_book_count_replica(bk_count)
        if len(bk_cost) != 0:
            update_cost_of_one_book_replica(bk_cost)
    except:
        print("No data to sync")
    return True

# API end point called by n-1 replicas so that they can sync their databases
# with the current database. This is called when they replicas are starting
# and before they go online.
# returns a list of changes to be synced to the replicas
@app.route('/resync_replicasdbs/<count>', methods=['GET'])
def resync_replica_dbs(count):
    bkcount = book_count_array.copy()
    bk_cost = book_cost_array.copy()
    data = {"book_count_array": bkcount, "book_cost_array": bk_cost}
    # clear pre-saved data
    book_count_array.clear()
    book_cost_array.clear()
    return jsonify(data)
...
def update(operation, change, book_number):
    ...
    if operation == 'update_cost':
        ...
        # Invalidate cache before making any changes to the price- uses server push
        print("Invalidating Cache for book {}.".format(book_number))
        m = requests.get('http://{}:{}/cache/invalidate/{}'.format(front_end_ip, front_end_port, book_number))
        print(m.json())
...
if __name__ == '__main__':
    ...
    # resync the databases before going online..
    # print("Resyncing the database ...")
    finished = resync_databases()

    if finished == True:
        resync_stop = time.time()
        resync_time = resync_stop - resync_start
        # run the server
        app.run(host=localhost, port=12350, threaded=True)
